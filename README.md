...
+ Directional 조명에 CSM 방식 적용 구현 필요
	...
	-> Directional Light CSM 구현 및 테스트 완료
	
	- 고정된 객체들에 대해서는 CSM 방식이 비효율적임  <--> 전체 씬에 있는 정적 객체들에 대한 셰도우맵을 미리 생성하고, 사용하는 것이 더 효율적
	-> CSM 을 사용하지 않고, 정적인 Directional ShadowMap을 사용하는 구조를 추가해야 함 // 다른 조명 타입들에도 비슷한 구조가 필요함
	
	
>>> 전역 조명에 대한 효율적인 접근은 정적인 Directional Static ShadowMap 과 동적인 객체들만 그리는 Directional CSM을 병합하는 구조
