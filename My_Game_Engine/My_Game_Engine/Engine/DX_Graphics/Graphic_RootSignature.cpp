...
ComPtr<ID3D12RootSignature> RootSignatureFactory::CreatePostFX(ID3D12Device* pd3dDevice)
{
    ...
    // === Shadow map textures (tN+7) ===
    D3D12_DESCRIPTOR_RANGE shadowRanges[3] = {};

    // CSM
    shadowRanges[0].RangeType = D3D12_DESCRIPTOR_RANGE_TYPE_SRV;
    shadowRanges[0].NumDescriptors = 1;
    shadowRanges[0].BaseShaderRegister = (UINT)GBufferType::Count + 7;
    ...
    shadowRanges[1].NumDescriptors = 1;
    shadowRanges[1].BaseShaderRegister = (UINT)GBufferType::Count + 8;
    ...
    shadowRanges[2].NumDescriptors = 1;
    shadowRanges[2].BaseShaderRegister = (UINT)GBufferType::Count + 9;
    ...
    params[ShadowMatrix_SRV].ParameterType = D3D12_ROOT_PARAMETER_TYPE_SRV;
    params[ShadowMatrix_SRV].Descriptor.ShaderRegister = (UINT)GBufferType::Count + 6;
    params[ShadowMatrix_SRV].Descriptor.RegisterSpace = 0;
    ...
    samplers[0] = CD3DX12_STATIC_SAMPLER_DESC(0, D3D12_FILTER_MIN_MAG_MIP_LINEAR);
    samplers[1] = CD3DX12_STATIC_SAMPLER_DESC(1, D3D12_FILTER_MIN_MAG_MIP_POINT);
    samplers[2] = CD3DX12_STATIC_SAMPLER_DESC(2, D3D12_FILTER_COMPARISON_MIN_MAG_LINEAR_MIP_POINT, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, D3D12_TEXTURE_ADDRESS_MODE_CLAMP, 0.0f, 16, D3D12_COMPARISON_FUNC_GREATER_EQUAL, D3D12_STATIC_BORDER_COLOR_OPAQUE_WHITE);
    ...
    rsDesc.NumStaticSamplers = _countof(samplers);
    rsDesc.pStaticSamplers = samplers;
    rsDesc.Flags = D3D12_ROOT_SIGNATURE_FLAG_ALLOW_INPUT_ASSEMBLER_INPUT_LAYOUT | D3D12_ROOT_SIGNATURE_FLAG_DENY_HULL_SHADER_ROOT_ACCESS | D3D12_ROOT_SIGNATURE_FLAG_DENY_DOMAIN_SHADER_ROOT_ACCESS;
...
ComPtr<ID3D12RootSignature> RootSignatureFactory::CreateShadowPass(ID3D12Device* pd3dDevice)
{
    ...
    params[ShadowMatrix_Index].Constants.ShaderRegister = 0;
    ...
    // b1: ObjectCBV
    params[ObjectCBV].ParameterType = D3D12_ROOT_PARAMETER_TYPE_CBV;
    params[ObjectCBV].Descriptor.ShaderRegister = 1;
    params[ObjectCBV].Descriptor.RegisterSpace = 0;
    params[ObjectCBV].ShaderVisibility = D3D12_SHADER_VISIBILITY_VERTEX;

    params[ShadowMatrix_SRV].ParameterType = D3D12_ROOT_PARAMETER_TYPE_SRV;
    params[ShadowMatrix_SRV].Descriptor.ShaderRegister = 0; // t0
    params[ShadowMatrix_SRV].Descriptor.RegisterSpace = 0;
